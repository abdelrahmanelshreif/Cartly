...
protocol RemoteDataSourceProtocol {
    ...
    func editInventoryQuantityExistingItemDraftOrder(updateQuantityEntity: UpdateQuantityEntity) -> AnyPublisher<DraftOrder?, Error>
}
...
final class RemoteDataSourceImpl: RemoteDataSourceProtocol {
    ...
    func editInventoryQuantityExistingItemDraftOrder(updateQuantityEntity: UpdateQuantityEntity) -> AnyPublisher<DraftOrder?, Error> {
        let request = APIRequest(
            withMethod: .PUT,
            /// force unwrap for draft order id cuz we have it already if we make put request
            withPath: "/draft_orders/\(updateQuantityEntity.orderID).json",
            withParameters: mapToQuantityEntity(updateQuantityEntity: updateQuantityEntity)
        )
        print("/draft_orders/\(updateQuantityEntity.orderID).json in remoooooote")
        return networkService.request(request, responseType: DraftOrderResponse.self)
            .map {
                $0?.draftOrder
            }
