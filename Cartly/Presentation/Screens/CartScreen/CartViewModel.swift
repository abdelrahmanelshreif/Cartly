            ...
            func updateQuantity(cartId: Int64, itemId: Int64, newQuantity: Int) {
                ...
                addToCartUseCase.execute(cartEntity: cartEntity)
                    .sink(
                        ...
                        receiveValue: { [weak self] success in
                            if let cartIndex = self?.cartItems.firstIndex(where: { $0.orderID == cartId }),
                               let itemIndex = self?.cartItems[0].itemsMapper.firstIndex(where: { $0.itemId == itemId }) {
                                self?.cartItems[cartIndex].itemsMapper[itemIndex].quantity = newQuantity
                            }
                            switch success {
                            case .Added:
                                print("Item quantity updated successfully")
