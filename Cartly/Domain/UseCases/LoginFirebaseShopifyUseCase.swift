        ...
        return authRepository.signIn(credentials: credentials)
            .flatMap { [weak self] userEmail -> AnyPublisher<ResultState<CustomerResponse?>, Never> in
                guard let self = self, let _ = userEmail else {
                    return Just(ResultState.failure(AuthError.firebaseloginFailed))
                        .eraseToAnyPublisher()
                }
                ...
                return self.customerRepository.getCustomers()
                    .map { customersResponse in
                        guard let customers = customersResponse?.customers else {
                            return ResultState.failure(AuthError.shopifyLoginFailed)
                        }
                        ...
                        if let customer = matchingCustomer {
                            ...
                            return ResultState.failure(AuthError.customerNotFoundAtShopify)
                        }
                    }
                    .catch { error in
                        Just(ResultState.failure(AuthError.shopifyLoginFailed))
                    }
                    .eraseToAnyPublisher()
            }
            .catch { error in
                Just(ResultState.failure(AuthError.firebaseloginFailed))
            }
