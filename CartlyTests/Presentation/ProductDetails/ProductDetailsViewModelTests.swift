...
struct MockProductData {
    static var productEntity: ProductInformationEntity {
        let variant1 = ProductInformationVariantEntity(id: 101, title: "S / Red", price: 19.99, size: "S", color: "Red", inventoryQuantity: 10, isAvailable: true)
        let variant2 = ProductInformationVariantEntity(id: 102, title: "M / Red", price: 19.99, size: "M", color: "Red", inventoryQuantity: 5, isAvailable: true)
        let variant3 = ProductInformationVariantEntity(id: 103, title: "S / Blue", price: 21.99, size: "S", color: "Blue", inventoryQuantity: 0, isAvailable: false)
...
struct ProducInformationtMapper {
    static var productToReturn: ProductInformationEntity = MockProductData.productEntity

    static func mapShopifyProductToProductView(_ product: Product) -> ProductInformationEntity {
...
class ProductDetailsViewModelTests: XCTestCase {
    ...
    func test_getProduct_whenSuccessful_updatesResultStateToSuccess() {
        ...
        sut.$resultState.dropFirst().sink { state in
            if case .success(let receivedProduct) = state {
                XCTAssertEqual(receivedProduct.id, MockProductData.productEntity.id)
                expectation.fulfill()
            }
        }.store(in: &cancellables)

        sut.getProduct(for: 1)
        mockGetProductUseCase.publisher.send(.success(Product(id: 1, title: "Test")))

        wait(for: [expectation], timeout: 1.0)
    }

    func test_getProduct_whenFails_updatesResultStateToFailure() {
        let expectation = XCTestExpectation(description: "State should be updated to failure")

        sut.$resultState.dropFirst().sink { state in
            if case .failure(let error) = state {
                XCTAssertEqual(error, .failedFetchingDataFromNetwork)
                expectation.fulfill()
            }
        }.store(in: &cancellables)

        sut.getProduct(for: 1)
        mockGetProductUseCase.publisher.send(.failure(AppError.failedFetchingDataFromNetwork))
    ...
    func test_getProduct_whenSourceIsCart_selectsSpecifiedVariant() {
        ...
        let expectation = XCTestExpectation(description: "Should select the variant specified from cart")
        ...
        sut.getProduct(for: 1, sourceisCart: true, cartVarientId: targetVariantId)
        mockGetProductUseCase.publisher.send(.success(Product(id: 1, title: "Test")))
    ...
    func test_updateSelectedVariant_whenValidSelection_enablesAddToCart() {
        ...
        sut.selectedSize = "M"; sut.selectedColor = "Red"
        XCTAssertNotNil(sut.selectedVariant)
        XCTAssertEqual(sut.selectedVariant?.id, 102)
        XCTAssertTrue(sut.isAddToCartEnabled)
    }

    func test_updateSelectedVariant_whenSelectionIsUnavailable_disablesAddToCart() {
        sut.currentProduct = MockProductData.productEntity
        sut.selectedSize = "S"; sut.selectedColor = "Blue"
        XCTAssertNotNil(sut.selectedVariant)
    ...
    func test_incrementQuantity_whenAtStockLimit_doesNotIncreaseQuantity() {
        sut.currentProduct = MockProductData.productEntity
        sut.selectedSize = "S"; sut.selectedColor = "Red"
        sut.quantity = 10
    ...
    func test_addToCart_whenQuantityExceedsStock_triggersAlert() {
        ...
        sut.addToCart()
        XCTAssertTrue(sut.triggerAlert)
        XCTAssertEqual(sut.alertMessage, "Requested quantity exceeds available stock")
        XCTAssertFalse(mockAddToCartUseCase.executeWasCalled)
    }
